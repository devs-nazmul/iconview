...
export default function Input({place, icon_start, className, query}){
	...
	return(
		<form action={"/icons"} className={cls(css.inputFlex, className)} onSubmit={ e => pathname === "/icons" && e.preventDefault() }>
			...
			<input className={css.input} name="q" type="text" value={query || filter.search} placeholder={place} onChange={e => {setEmpty(e.target.value); setFilter({search: e.target.value})}} />
